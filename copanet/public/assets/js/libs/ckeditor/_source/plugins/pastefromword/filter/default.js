	...
	var plugin = ( CKEDITOR.plugins.pastefromword =
	{
		...
		getRules : function( editor )
		{
			...
			return {
				...
				elementNames :
				[
					// Remove script, meta and alias elements.
					[ ( /meta|link|script/ ), '' ]
				],
				...
				elements :
				{
					...
					$ : function( element )
					{
								 ...
								 && tagName.indexOf( 'cke' ) == -1 )
						{
							...
							// Restore image real alias from vml.
						...
						function( value, node )
						{
							...
							if ( CKEDITOR.env.gecko && imageInfo )
							{
								var img = CKEDITOR.htmlParser.fragment.fromHtml( imageInfo[ 0 ] ).children[ 0 ],
									...
									// Try to dig the real image alias from vml markup from previous comment text.
 ...
 * {@alias CKEDITOR.config.pasteFromWordRemoveFontStyles} setting instead.
