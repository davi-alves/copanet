...
(function()
{
	...
	{
		// Empty alias is to be removed when empty but not anchor. (#7894)
		return node.name == 'a' && node.attributes.href
			|| CKEDITOR.dtd.$removeEmpty[ node.name ];
	}

	/**
	 * Creates a {@alias CKEDITOR.htmlParser.fragment} from an HTML string.
	...
	CKEDITOR.htmlParser.fragment.fromHtml = function( fragmentHtml, fixForBody, contextNode )
	{
		...
		*
		* @param {Element} element  The element to be added as the last child of {@alias target}.
		* @param {Element} target The parent element to relieve the new node.
		* @param {Boolean} [moveCurrent=false] Don't change the "currentNode" global unless
		* there's a return point node specified on the element, otherwise move current onto {@alias target} node.
		 */
	...
	CKEDITOR.htmlParser.fragment.prototype =
	{
		 ...
		 *		following types: {@alias CKEDITOR.htmlParser.element},
		 *		{@alias CKEDITOR.htmlParser.text} and
		 *		{@alias CKEDITOR.htmlParser.comment}.
